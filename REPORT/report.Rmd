...
author: ""Joshah (S2138074), Kristian (S2043845), Sharmini (S2114595), Min Jie (S2104353), Khai Mun (S2027890)""
date: "15 June 2022"
output: 
  html_document:
    theme: bootstrap
...
`r text_spec("**1. Subset the data**", background = "#D05A6E", color = "white", bold = T)`
```{r results='hide'}
df5 = subset(df3, select = c("popularity","danceability","loudness","valence","energy"))
```

`r text_spec("**2. Overview of the dataset**", background = "#D05A6E", color = "white", bold = T)`
```{r, echo=FALSE}
datatable(df5, extensions = 'Buttons', rownames=FALSE, filter = 'top', options = list(pageLength = 5, dom ='Bfrtip', 
                         buttons =c('copy', 'csv','excel','pdf', 'print'), class = 'hover cell - border stripe'))
```

`r text_spec("**3. Check the correlation**", background = "#D05A6E", color = "white", bold = T)`
```{r}
plot(df5,col=c("red","blue"))
```

`r text_spec("**4. Fit a model with multiple regression**", background = "#D05A6E", color = "white", bold = T)`
```{r}
multiple.regression = lm(popularity ~ danceability + energy + loudness + valence, data = df5)
```
...
On the regression section, the model shows that popularity is highly dependent on danceability, energy, loudness and valence.
