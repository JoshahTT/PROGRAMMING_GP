...
```{r results='hide'}
hist(df3$popularity)
##dependent variable not normal, failed and cannot proceed with linear regression analysis. We will consider non-linear regressions
```
...
`r text_spec("**4. Fit a model with multiple regression**", background = "#D05A6E", color = "white", bold = T)`
```{r}
multiple.regression <- gam(popularity ~ s(danceability) + s(liveliness) + s(valence), data=df5)
```
...
## **CONCLUSION**
**In conclusion, the experiment have shown significant insights on song recommendation to users based on the extracted component songs listened collectively by the respective user. The data cleaning portion required more attention which raises concerns on the initial data collection process. It is also important to note that training the machine learning models would be required in order to produce accurate and appropriate results. We continue to invite further research into data collected from other years, example 2019, 2018 etc. Overall, machine learning models with classification and regression analysis have the capability to futher recommend song preference tailored to a user's historical records.
